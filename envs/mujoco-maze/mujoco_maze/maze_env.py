...
class MazeEnv(gym.Env):
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        goal_pos = self._task.goals[0].pos
        
        return np.concatenate([obs, *view, np.array([self.t * 0.001]), goal_pos])
    ...
    def reset(self) -> np.ndarray:
        ...
        pos = self.inter_context[random_idx, :] + 0.1*self.np_random.randn(2, ) # add some randomness
        self._task.set_goal(pos)
        self.set_marker()
    ...
    def set_marker(self) -> None:
        for i, goal in enumerate(self._task.goals):
            idx = self.wrapped_env.sim.model.site_name2id(f"goal_site{i}")
            self.wrapped_env.sim.model.site_pos[idx][: len(goal.pos)] = goal.pos
